...
public:
	...
	Arguments:
		...
		boxType identify whether to draw OBB - 0 AABB - 1 Both - 2 */
	void SetVisible(bool a_bVisible, String a_sInstanceName = "ALL", int boxType = 2);
	/*Sets the Color of the specified Instance
	Arguments:
		a_v3Color sets the color of the shape to be drawn
		a_sInstanceName identify the shape if ALL is provided then it applies to all shapes
		boxtype identify whether to draw OBB - 0 AABB - 1 Both - 2*/
	void SetColor(vector3 a_v3Color, String a_sInstanceName = "ALL", int boxType = 2);
...
private:
	...
	Arguments
		...
		boxType identify whether box is OBB - 0 or ABB - 1*/
	bool CheckForNameInList(String a_sName, int boxType);
};
